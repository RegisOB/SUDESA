...
fig2.1 <- ggplot(data.baseline.sub1, 
                 aes(x = AgeGr, y = n, fill = INDIVIDUAL_INFO_GENDER)) +
  ylab('Count(n)') + xlab('Age groups (Yrs)')+ labs(fill = 'Sex')+
  geom_bar(data = subset(data.baseline.sub1, INDIVIDUAL_INFO_GENDER == "F"),
           stat = "identity") +
  geom_bar(data = subset(data.baseline.sub1, INDIVIDUAL_INFO_GENDER == "M"),
           stat = "identity",
           position = "identity",
           mapping = aes(y = -n)) +
